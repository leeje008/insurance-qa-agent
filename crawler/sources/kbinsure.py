"""crawler.sources.kbinsure - KB손해보험 약관 크롤러.

KB손해보험 약관 공시 페이지에서 보험약관 PDF를 수집한다.
robots.txt: 네이버봇만 명시적 허용 — 사전 협의 권장.
...
DISCLOSURE_URL = f"{BASE_URL}/CG802030001.ec"


class KBInsureCrawler(BaseCrawler):
    """KB손해보험 약관 크롤러.

    robots.txt가 제한적이므로 check_robots_txt() 결과에 따라
    자동으로 크롤링을 중단할 수 있다.
    """

    source_name = "kbinsure"
    ...
    async def fetch_product_list(self) -> list[ProductMeta]:
        ...
        if not allowed:
            logger.warning("KB손보 robots.txt 차단 — 크롤링 중단")
            return products
        ...
        async with async_playwright() as pw:
            ...
            logger.info("KB손보 약관 페이지 로드: %s", DISCLOSURE_URL)
            await page.goto(
                DISCLOSURE_URL, wait_until="networkidle", timeout=30000,
            )
        ...
        logger.info("KB손보 약관 총 %d건 목록 수집 완료", len(products))
        return products
