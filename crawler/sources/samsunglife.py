"""crawler.sources.samsunglife - 삼성생명 약관 크롤러.

삼성생명 공시실에서 생명보험 약관 PDF를 수집한다.
JS SPA 기반이므로 Playwright 사용.
...
class SamsungLifeCrawler(BaseCrawler):
    """삼성생명 약관 크롤러."""

    source_name = "samsunglife"
    base_url = BASE_URL
    ...
    async def fetch_product_list(self) -> list[ProductMeta]:
        ...
        async with async_playwright() as pw:
            ...
            # 공시실 접근
            disclosure_urls = [
                f"{BASE_URL}/individual/products/disclosure/clause.do",
        ...
        logger.info("삼성생명 약관 총 %d건 수집 완료", len(products))
        return products
